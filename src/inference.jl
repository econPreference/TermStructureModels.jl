...
"""
function tuning_hyperparameter(yields, macros, ρ, upper=[12, 1, 100]; medium_τ=12 * [1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5], maxtime=0.0, mSR_threshold=3)
    ...
    function negative_log_marginal(input, p_max_)
        ...
        if sum(mSR .< mSR_threshold) / length(mSR) < 0.5
            return Inf
        end
    ...
    ux = 0.0 .+ [upper[1]; upper[2]; 1; 2; upper[3]; 100; size(yields, 1)]
...
"""
function sparse_precision(saved_θ, T)
    ...
    @showprogress 1 "Imposing sparsity on precision..." for iter in 1:iteration
        ...
        std_ = sqrt.(diag(ΩFF_))
        glasso_results = rcopy(rcall(:EBICglasso, ΩFF_, T, threshold=true, returnAllResults=true))

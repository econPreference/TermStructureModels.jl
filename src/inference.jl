...
"""
function tuning_hyperparameter(yields, macros, τₙ, ρ; populationsize=30, maxiter=0, medium_τ=12 * [1.5, 2, 2.5, 3, 3.5], lag=1, upper_q1=1, upper_q4=100, upper_q5=100, σ²kQ_infty=1, mSR_mean=Inf, upper_Ω0=2)

    dQ = dimQ()
    dP = dQ + size(macros, 2)
    PCs, ~, Wₚ = PCA(yields, lag)
    AR_re_var_vec = [AR_res_var([PCs macros][:, i], lag) for i in 1:dP]
    lx = 0.0 .+ [eps(); eps(); 1; eps(); eps(); 1; eps() * ones(dP)]
    ux = 0.0 .+ [upper_q1; 1; 10; upper_q4; upper_q5; size(yields, 1); upper_Ω0 * AR_re_var_vec]
...
"""
function tuning_hyperparameter_MOEA(yields, macros, τₙ, ρ; populationsize=100, maxiter=0, medium_τ=12 * [1.5, 2, 2.5, 3, 3.5], lag=1, upper_q1=1, upper_q4=100, upper_q5=100, σ²kQ_infty=1, upper_Ω0=2)

...
"""
function tuning_hyperparameter(yields, macros, τₙ, ρ; medium_τ=12 * [1.5, 2, 2.5, 3, 3.5], maxtime=0.0, upper_lag=9, upper_q1=1, upper_q45=100, upper_ΩFF=2, weight=0.0, mSR_mean=1.0)
    ...
    function negative_log_marginal(input, p_max_)
        ...
        q[2] = q[1] * q[2]
        ν0 = input[7] + dP + 1
        Ω0 = input[8:end-1] * input[7]
        σ²kQ_infty = input[end]
    ...
    lx = 0.0 .+ [1; 0; 0; 0; 0; 0; 0; zeros(dP); 0.0001^2]
    ux = 0.0 .+ [upper_lag; upper_q1; 1; 10; upper_q45; upper_q45; size(yields, 1); upper_ΩFF * starting[8:end-1]; 1]
    obj_EA(x) = negative_log_marginal(x, Int(ux[1]))
    ss = MixedPrecisionRectSearchSpace(lx, ux, [0; -1ones(Int64, 6 + dP + 1)])
    ...
    Ω0 = best_candidate(EA_opt)[8:end-1] * best_candidate(EA_opt)[7]
    σ²kQ_infty = best_candidate(EA_opt)[end]
...
"""
function tuning_hyperparameter_mSR(yields, macros, τₙ, ρ; medium_τ=12 * [1.5, 2, 2.5, 3, 3.5], maxstep=10_000, weight=1.0, mSR_mean=1.0, upper_lag=9, upper_q1=1, upper_q45=100, upper_ΩFF=2)
    ...
    Ω0 = bo1_solution[8:end-1] * bo1_solution[7]
    σ²kQ_infty = bo1_solution[end]
...
"""
function mSR_ML_frontier(EA_opt, dM; mSR_mean=1.0)

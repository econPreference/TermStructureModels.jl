...
"""
function tuning_hyperparameter(yields, macros, τₙ, ρ; medium_τ=12 * [1.5, 2, 2.5, 3, 3.5], maxtime=0.0, mSR_median=Inf, mSR_tail=Inf, upper_lag=12, upper_q1=1, upper_q4=100, upper_ΩFF=2, σ²kQ_infty=1)
    ...
    starting = [1, upper_q1 / 2, 1, 2, upper_q4 / 2, 1, 1]
    for i in 1:dP
        push!(starting, AR_res_var([PCs macros][:, i], Int(upper_lag)))
    end
    lx = 0.0 .+ [1; 0; 0; 0; 0; 0; 0; zeros(dP)]
    ux = 0.0 .+ [upper_lag; upper_q1; 1; 6; upper_q4; 100; size(yields, 1); upper_ΩFF * starting[8:end]]

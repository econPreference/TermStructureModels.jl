...
* Output(2): ψ0, ψ
    - posterior samples
"""
function post_ψ_ψ0(μϕ_const, ρ, prior_κQ_, τₙ, Wₚ; ϕ, ψ0, ψ, ηψ, q, σ²FF, ν0, Ω0, fix_const_PC1)

    R"library(GIGrvg)"
    dP = size(ψ, 1)
    p = Int(size(ψ, 2) / dP)
    priormean_ϕ0_ = mean.(prior_ϕ0(μϕ_const, ρ, prior_κQ_, τₙ, Wₚ; ψ0, ψ, q, ν0, Ω0, fix_const_PC1))
    post_ψ = similar(ψ)
    post_ψ0 = similar(ψ0)

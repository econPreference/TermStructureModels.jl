...
===
"""
function prior_ϕ0(ρ::Vector; κQ, ψ0, ψ, q, ν0, Ω0)

    dP, dPp = size(ψ) # dimension & #{regressors}
    p = dPp / dP # the number of lags
    ϕ0 = Matrix{Any}(undef, dP, dPp + 1)
    dQ = GQ_XX() # reduced dimension of yields

    ϕ0[1:dQ, 2:(dQ+1)] = GQ_XX(; κQ)
    for i in 1:dQ
        ϕ0[i, 1] = Normal(0, ψ0[i] * q[4])
        for l = 2:p
            for j in 1:dP
                ϕ0[i, 1+dP*(l-1)+j] = Normal(0, ψ[i, dP*(l-1)+j] * Minnesota(l, i, j; q, ν0, Ω0))
            end
        end
    end
    for i in (dQ+1):dP
        ...
        for l = 1:p
            for j in 1:dP
                if i == j && l == 1
                    ϕ0[i, 1+dP*(l-1)+j] = Normal(ρ[i], ψ[i, dP*(l-1)+j] * Minnesota(l, i, j; q, ν0, Ω0))
                else
                    ϕ0[i, 1+dP*(l-1)+j] = Normal(0, ψ[i, dP*(l-1)+j] * Minnesota(l, i, j; q, ν0, Ω0))
                end
            end
...
function prior_κQ(medium_τ=12 * [1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]) # Default candidates are one to five years
    ...
    for i in 1:N # calculate the maximizer κQ
        obj(κQ) = 1200 * dcurvature_dτ(; κQ, medium_τ[i])
...
function prior_γ(yields)
    dQ = GQ_XX() # #(latent factors in the bond market)
    ...
    return 1 / mean(OCs - PCs * ((PCs'PCs) \ (PCs'OCs)))
end

# code source: https://github.com/samozm/Distributions.jl/blob/master/src/univariate/continuous/generalizedinversegaussian.jl

module GIG

using Distributions, Statistics, FillArrays, LinearAlgebra, Random, SpecialFunctions
...
import StatsBase: kurtosis, skewness, entropy, mode, modes, fit, kldivergence, loglikelihood, dof, span, params, params!
...
function rand(rng::AbstractRNG, d::GeneralizedInverseGaussian)
    (a, b, p) = params(d)
    α = sqrt(a / b)
    β = sqrt(a * b)
    λ = abs(p)

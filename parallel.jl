...
par_posterior = pmap(i -> posterior_sampler(Array(yields[:, 2:end]), Array(macros[:, 2:end]), τₙ, ρ, Int(iteration / n_core), tuned; sparsity=issparsity, init_param=init_θ[(floor.(Int, collect(range(burn_in, burn_in + init_range, length=n_core))))[i]], force_disp=ceil(Int, iteration / 100)), 1:n_core)
# rmprocs(2:(n_core+1))
for i in 1:n_core
    if i == 1

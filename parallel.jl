...
tuned = HyperParameter(p=tuned.p, q=tuned.q, ν0=tuned.ν0, Ω0=tuned.Ω0, σ²kQ_infty=1)
save("tuned.jld2", "tuned", tuned)
tuned = load("tuned.jld2")["tuned"]
# tuned, EA_opt = tuning_hyperparameter_mSR(Array(yields[:, 2:end]), Array(macros[:, 2:end]), τₙ, ρ; maxstep=50_000, weight=1E+5)
# save("tuned_pf.jld2", "tuned", tuned, "EA_opt", EA_opt)
# tuned = load("tuned_pf.jld2")["tuned"]
# EA_opt = load("tuned_pf.jld2")["EA_opt"]
# tuned, p = mSR_ML_frontier(EA_opt, size(macros, 2) - 1; mSR_mean=1.0)
# Plots.plot(p)

...
begin ## Data: macro data
    ...
    for i in axes(macros[:, 2:end], 2) # i'th macro variable (excluding date)
        ...
        elseif rcopy(rcall(:describe_md, names(macros[:, 2:end])))[:, :fred][i] ∈ ["HOUST", "PERMIT", "M2REAL", "REALLN", "WPSFD49207", "CPIAUCSL", "CUSR0000SAD", "PCEPI", "CES0600000008", "CES0600000007"]
            macros[2:end, i+1] = log.(macros[2:end, i+1]) - log.(macros[1:end-1, i+1]) |> x -> 100 * x
            macros[2:end, i+1] = macros[2:end, i+1] - macros[1:end-1, i+1]
            ρ[i] = 0.0
        else
            macros[2:end, i+1] = log.(macros[2:end, i+1]) - log.(macros[1:end-1, i+1]) |> x -> 100 * x
...
calibration_kQ_infty(0.04, Array(yields[p_max-lag+1:end, 2:end]), τₙ, lag; κQ=0.0609)

if step == 0 ## Drawing pareto frontier

    par_tuned = @showprogress 1 "Tuning..." pmap(1:p_max) do i
        tuning_hyperparameter_MOEA(Array(yields[p_max-i+1:end, 2:end]), Array(macros[p_max-i+1:end, 2:end]), τₙ, ρ; lag=i, μkQ_infty=μkQ_infty, σkQ_infty=σkQ_infty)
    end
...
elseif step == 1 ## Tuning hyperparameter
    ...
    par_tuned = @showprogress 1 "Tuning..." pmap(1:p_max) do i
        ...
        tuning_hyperparameter(Array(yields[p_max-i+1:end, 2:end]), Array(macros[p_max-i+1:end, 2:end]), τₙ, ρ; lag=i, upper_q1=upper_q1, upper_q4=upper_q4, upper_q5=upper_q5, μkQ_infty=μkQ_infty, σkQ_infty=σkQ_infty, mSR_tail=mSR_tail, initial=x0)
    end

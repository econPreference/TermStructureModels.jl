...
elseif step == 1 ## Tuning hyperparameter
    ...
    par_tuned = @showprogress 1 "Tuning..." pmap(1:p_max) do i
        ...
        if isfile("tuned_pf.jld2")
            ...
            tuned_ = pf_input[i][findall(x -> x < mSR_upper, pf[i][2])]
        ...
        tuning_hyperparameter(Array(yields[p_max-i+1:end, 2:end]), Array(macros[p_max-i+1:end, 2:end]), τₙ, ρ; lag=i, maxiter_global=maxiter_global, upper_q1=1, upper_q4=1, upper_q5=1, σ²kQ_infty=0.02^2, mSR_upper=mSR_upper, initial=x0, maxiter_local=maxiter_local)
    end
